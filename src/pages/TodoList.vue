   ...
   <div> 
    ...
    <ul class="list">
      <li class="list-item" v-for="(task, index) in tasks" :key="task.id">
        <div class="list-name">{{task.name}}</div><button class="btn btn-primary" @click="removeTask(index)">x</button>
      </li>
    </ul>
    <div class="add-task">
...
export default {
  setup(){
    ...
    const maxTaskId = computed(() => {
        return tasks.value.length <= 0 ? -1 : Math.max(...tasks.value.map(a => a.id));
    });
